...
class Breadcrumbs_Presenter extends Abstract_Indexable_Presenter {
	...
	protected function crumb_to_link( $breadcrumb, $index, $total ) {
		...
		if (
			...
			$link      .= '<a class="breadcrumbs__item breadcrumbs__link" href="' . \esc_url( $breadcrumb['url'] ) . '"' . $title_attr . '>' . $text . '</a>';
		}
		elseif ( $index === ( $total - 1 ) ) {
			...
			$link .= '<' . $inner_elm . ' class="breadcrumbs__item" aria-current="page">' . $text . '</' . $inner_elm . '>';
